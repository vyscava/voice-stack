#!/bin/bash
# Voice Stack TTS Service Update Script
#
# This script updates the Voice Stack TTS service after code changes.
# It stops the service, updates dependencies, and restarts the service.
#
# Usage:
#   sudo ./update-tts.sh               # Update system service
#   ./update-tts.sh                     # Update user service
#
...
# Print banner
echo "=========================================="
echo "Voice Stack TTS Service Update"
echo "=========================================="
echo ""
...
if ! service_exists "$SERVICE_NAME"; then
    ...
    log_info "Please install it first with: ./scripts/install-tts.sh"
    exit 1
fi
...
if [ "$EUID" -eq 0 ] && [ -n "$SUDO_USER" ]; then
    if ! sudo -u "$SUDO_USER" git diff-index --quiet HEAD --; then
        ...
        sudo -u "$SUDO_USER" git stash push -m "Auto-stash by update-tts.sh"
        STASHED=true
    fi
    ...
    if ! git diff-index --quiet HEAD --; then
        log_warn "Uncommitted changes detected. Stashing them..."
        git stash push -m "Auto-stash by update-tts.sh"
        STASHED=true
    fi
    git pull --rebase
fi
