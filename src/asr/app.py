...
from asr.api.api_v1.api import api_router
...
async def idle_timeout_checker() -> None:
    """
    Background task that periodically checks if the ASR model should be unloaded due to inactivity.

    Runs every 60 seconds and checks if the model has been idle for longer than
    ASR_IDLE_TIMEOUT_MINUTES. If so, unloads the model to free resources.
    """
    from asr.engine_factory import _engine

    while True:
        try:
            await asyncio.sleep(60)  # Check every minute

            if _engine is not None and settings.ASR_IDLE_TIMEOUT_MINUTES > 0:
                # Check and unload if idle
                was_unloaded = _engine.check_and_unload_if_idle()
                if was_unloaded:
                    logger.info("ASR model unloaded due to inactivity")
...
async def app_init(app: FastAPI) -> AsyncIterator[None]:
    ...
    if settings.ASR_IDLE_TIMEOUT_MINUTES > 0:
        idle_task = asyncio.create_task(idle_timeout_checker())
        logger.info(f"Idle timeout checker started (timeout: {settings.ASR_IDLE_TIMEOUT_MINUTES} minutes)")
    else:
        idle_task = None
        logger.info("Idle timeout disabled (ASR_IDLE_TIMEOUT_MINUTES=0)")
    ...
    try:
        ...
        if hasattr(engine_factory, "_semaphore") and engine_factory._semaphore is not None:
            ...
            max_concurrent = settings.ASR_MAX_CONCURRENT_REQUESTS
...
async def detailed_health() -> JSONResponse:
    ...
    from asr.engine_factory import _engine, _semaphore
    ...
    response_data = {
        ...
        "config": {
            "idle_timeout_minutes": settings.ASR_IDLE_TIMEOUT_MINUTES,
